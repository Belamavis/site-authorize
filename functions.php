...
# ##############################
# Скрипт с функциями авторизации
# ##############################

	# connectToDB - подключение к базе данных
	function connectToDB() {
			global $link, $dbuser, $dbpass, $dbname;
	...
	function newUser($login, $password) {
			...
			$result = mysql_query($query, $link) or die ("Died inserting login info into db. Error returned if an: ".mysql_error());
			return true;
		}

	# displayErrors - вывод массива ошибок	
	...
	# checkLoggedIn - проверка авторизации
	...
	# checkPass - проверка пользователя в БД во время авторизации
	...
	# cleanMemberSession - авторизация пользователя
	...
	# field_validator - валидация содержимого полей
